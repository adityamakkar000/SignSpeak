...
class ModelInfo:
  ...
  def get_accuracy(self, cm: List[List[int]]) -> Tuple[List[float], List[float]]:
    ...
    FP = np.sum(cm, axis = 0) - np.diag(cm) # False Positives are the sum of the column minus the diagonal
    FN = np.sum(cm, axis=1) - np.diag(cm) # False Negatives are the sum of the row minus the diagonal
