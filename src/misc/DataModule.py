...
class getDataset(Dataset):
    ...
    def __init__(
        self, x: List[List[List[int]]], x_padding: List[List[list[bool]]], y: List[int]
    ):
...
class ASLDataModule(L.LightningDataModule, Dataset):
    ...
    def setup(self, stage: str):
        ...
        ]  # range set to 20-30 for numbers/ do not remove 1-10
        indexes = [i for i, s in enumerate(y) if s not in number]  # remove numbers
        y = [s for i, s in enumerate(y) if s not in number]
        stoi = {
            s: i for i, s in enumerate(sorted(set(y)))
        }  # assign indexes to each possible class (a - z, 1-10)
        encode = lambda s: stoi[
            s
        ]  # inline function to covert character to class number
    ...
    """
        ...
        x = [
            i[~np.isnan(i)]
            for i in data.iloc[:, 2 : self.time_steps * 5 + 2].to_numpy()
        ...
        kfold = StratifiedKFold(
            n_splits=self.splits, shuffle=True, random_state=self.seed
        )
...
""" Test if DataModule provides the same batches across runs """
if __name__ == "__main__":

    for k in range(5):
        dm = ASLDataModule(k, 5, 1337, 79, 5, 32)
        dm.setup("asda")
        ...
        for i in train_DL:
            with open("test2.txt", "a") as f:
                f.write(
                    f" {[str(i) for i in i['measurement']]} \n {[str(i) for i in i['measurement_padding']]} \n {[str(i) for i in i['label']]}"
                )
